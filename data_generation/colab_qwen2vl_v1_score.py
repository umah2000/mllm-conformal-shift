...
TEMPERATURE = 1.1
TOP_P = 0.95
OUT_PATH = f"scores_{DATASET}_gpu_v2.csv"
# -----------------------------------------
...
def generate_k_samples(image, prompt, k):
    """k SEQUENTIAL independent generate() calls (num_return_sequences=1 each).
    NOTE: an earlier version used a single generate(..., num_return_sequences=k)
    call to batch all k samples at once. That produced near-identical samples
    for ~97% of examples — a known issue where multimodal input expansion
    (pixel_values / image_grid_thw) for num_return_sequences>1 does not
    correctly decorrelate the sampled sequences for this model. Calling
    generate() k separate times is slower but reliably independent."""
    messages = [{
        "role": "user",
...
if Path(OUT_PATH).exists():
    ...
    with open(OUT_PATH, "w", newline="") as f:
        csv.writer(f).writerow(
            ["id", "order", "nonconformity_score", "correct", "majority_answer", "gt_answer", "samples"])
...
for idx, ex in enumerate(examples):
    ...
    try:
        ...
        samples = generate_k_samples(ex["image"], prompt, K)
        ...
        if idx < 5:
            print(f"  [{ex['id']}] samples: {samples}", flush=True)
            if len(set(samples)) == 1:
                print(f"  [{ex['id']}] WARNING: all {K} samples identical again — "
                      f"if this keeps happening, flag it.", flush=True)
    ...
    if idx % 10 == 0:
        ...
        print(f"{idx+1}/{len(examples)} done | {rate:.1f}s/ex | ETA {eta_min:.1f} min")
